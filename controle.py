...
def salvar_dados():
    ...
    cursor = conexao.cursor()
    cursor.execute("UPDATE produto SET id='{}', produto='{}', preco='{}', estoque='{}' WHERE id={}".format(id, produto, preco, estoque, numero_id))
...
def inserir():
    ...
    preco = formulario.txtPreco_2.text()
    estoque = formulario.txtEstoque.text()

    cursor = conexao.cursor()
    comando_SQL = 'INSERT INTO produto(produto, preco, estoque) VALUES (%s, %s, %s)'
    dados = (str(produto), str(preco), str(estoque))
    cursor.execute(comando_SQL, dados)
    conexao.commit()

    formulario.txtProduto.setText('')
    formulario.txtPreco_2.setText('')
